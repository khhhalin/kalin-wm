...
- Cross-monitor moves (both decided 2026-07-15, shipped 2026-07-17 — see Phase 2 below for as-built detail): drag hand-off (Super+drag crossing the physical boundary reassigns `c->mon` and re-bases the window under the cursor through the new camera) + an explicit send-to-monitor bind (`move-monitor`, teleport to the other camera's center). Cross-monitor [[connection-graph]] edges are severed on hand-off (lines through two different cameras are meaningless).
...
2. **Hand-off — DONE (2026-07-17).** As-built:
   - *Drag hand-off* (`motionnotify()`'s CurMove/CurMoveSolo branch, dwl.c): when `xytomon(cursor)` differs from `grabc->mon` mid-drag, `setmon()` reassigns the holder before the tick's transform. No explicit re-basing math: `grabcx/grabcy` are window-internal *world* offsets, so the existing `SCREEN_TO_WORLD(grabc->mon, cursor)` line re-derives a world position that puts the grabbed point back under the cursor through the new camera. World size is preserved (on-screen size snaps to the new camera's zoom). The component-glide delta is suppressed on the hand-off tick — `move_dx/dy` then contain the camera re-basing jump, not cursor motion. Camera-bypassed clients (docked/fullscreen/maximized) skip hand-off and keep the old release-time `setmon()` in `buttonpress()`.
   - *Send-to-monitor* (`tagmon()`, still the `move-monitor` bind, `Super+Shift+less/greater` — no new ACT_* was needed): `setmon()` to `dirtomon()`'s target, then teleport (`resize()`, no animation) centered on what the target camera currently shows (`SCREEN_TO_WORLD(m, m->m center)`), then sever + `persistence_save()` (same persist-on-drop rule as drag release). Camera-bypassed clients switch holder without the teleport.
   - *Severing*: both paths call `sever_cross_monitor_edges(Client *c)` (new, `connection_graph.c`, declared in `kalin.h` + dwl.c's own forward-decl block) — clears every direct edge between the moved window and neighbors held by a different monitor, symmetrically; see [[connection-graph]].
   - Verified: build + unit tests green; single-monitor path byte-identical (hand-off condition never true, old `tagmon` no-op preserved via early return). **Dual-output GPU verification pending (keeper).**
...
4. **UI polish — DONE (2026-07-18)** except wallpaper (analysis only). As-built:
   - *Off-screen indicators*: per-monitor marker sets — each monitor flags only its own held windows against its own camera, clamped to `m->m` (see [[off-screen-indicators]] for the lazy-create/GC-against-`mons` lifecycle and the zoom-scaled footprint test).
   - *Overview*: `Super+O` on a different monitor moves the overview there (restore old, open on selmon); clicking another monitor's window dismisses-with-restore instead of dropping the saved camera; dangling `overview_mon` after monitor destroy fixed (see [[overview-mode]]).
   - *Wallpaper*: still selmon-follow. True per-monitor parallax is impossible with unclipped rect-tile trees (sub-tile overhang spills onto the neighbor output); the workable design — pattern rendered to a `wlr_buffer`, one clipping `wlr_scene_buffer` per monitor panned via its source box — needs keeper-level `kalin.h`/dwl.c cleanup and GPU verification; full analysis in [[wallpaper]].
   - Vault sweep done: [[viewport]], [[infinite-canvas]], [[world-coordinates]] speak the per-monitor camera model.
