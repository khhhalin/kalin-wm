...
clean:
	rm -rf $(BUILD_DIR)
	rm -f kalin-wm
	rm -f tests/test_client_lifecycle tests/test_binds tests/test_growth_overlap tests/test_viewport_ops tests/test_rail tests/test_float tests/test_overlay tests/test_persistence tests/test_window_shader_math tests/*.gcda tests/*.gcno tests/*.gcov
...
test-unit:
	...
	@echo "=== Running Persistence (rail + overlay) Tests ==="
	@gcc -std=c99 -Wall -Wextra -Wshadow -O1 -g -o tests/test_persistence code/tests/test_persistence.c -lm && tests/test_persistence
