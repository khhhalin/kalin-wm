...
* Build was broken: `same_column_x()` was defined in both `modules/crop/crop_mode.c`
  and `modules/layout/layout_world.c`, which dwl.c `#include`s into one translation
  unit, causing a `redefinition` error. The definition now lives only in the layout
  module, forward-declared in dwl.c.
* Buffer scaling silently disabled: the child-node scan in `client_set_buffer_scale()`
  compared `node->parent` (a `wlr_scene_tree *`) against `&c->scene_surface->node`
  (a `wlr_scene_node *`), which was always unequal, so the loop body never ran and no
  buffer ever received a destination size. Now compares against the tree itself.
* Removed dead code and several warnings: unused `infinite_topmost_y()`,
  `column_rightmost_edge` declaration, `COLUMN_SPACING` macro; signed/unsigned and
  shadowing warnings in `client_set_buffer_scale()`, `persistence.c`, and the column
  layout; and an ignored `write()` result in the spawn error path.
