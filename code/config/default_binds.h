/*
 * default_binds.h - the built-in bind DSL, written to
 * ~/.config/kalin-wm/binds.conf on first run ONLY — NOT a fallback for a
 * broken/incomplete user file (there used to be one; binds_init() now
 * die()s instead, deliberately, so a stale config can't silently drift
 * out of sync with the compositor's evolving action set — see the ledger).
 * Kept in a shared header so the parser unit test
 * (test_shipped_default_parses) can verify the whole thing parses AND
 * covers every registered action (bind_check_coverage()) — guarding
 * against a fresh install ever booting with an incomplete default.
 */
#ifndef KALIN_DEFAULT_BINDS_H
#define KALIN_DEFAULT_BINDS_H

static const char DEFAULT_BINDS[] =
"# kalin-wm bindings. Reloaded live when saved.\n"
"# Syntax: bind [tap|hold] Mods+Key -> action args    (# starts a comment)\n"
"# Mods: Super Shift Ctrl Alt.  See docs for the full grammar.\n"
"\n"
"bind Super+p       -> spawn foot --app-id=kalin-launcher -e kalin-launch\n"
"bind Super+t       -> spawn foot\n"
"bind Super+o       -> toggle-overview\n"
"bind Super+Print   -> screenshot\n"
"bind Super+Shift+s -> screenshot-ui\n"
"bind Super+v       -> spawn foot -e kalin-clip-picker\n"
"bind Super+j       -> focus-stack next\n"
"bind Super+k       -> focus-stack prev\n"
"bind Super+q       -> close\n"
"bind Super+c       -> crop\n"
"bind Super+Shift+r -> crop-cancel\n"
"bind Super+e       -> toggle-fullscreen\n"
"bind Super+f       -> fit-width\n"
"bind Super+Shift+f -> fit-height\n"
"bind Super+m       -> toggle-maximized\n"
"bind Super+Shift+t -> toggle-ontop\n"
"bind Super+Shift+o -> toggle-overlap\n"
"bind Super+n       -> toggle-minimized\n"
"bind Super+i       -> toggle-paper\n"
"bind Super+a       -> spawn kalin-claude\n"
"bind Super+h       -> spawn kalin-hx\n"
"bind Super+y       -> paper-yellow 0.1\n"
"bind Super+Shift+y -> paper-yellow -0.1\n"
"bind Super+grave   -> toggle-scratchpad foot --app-id=kalin-scratchpad\n"
"bind Super+d       -> opacity -0.1\n"
"bind Super+Shift+d -> opacity 0.1\n"
"bind Super+comma   -> focus-monitor left\n"
"bind Super+period  -> focus-monitor right\n"
"bind Super+Shift+less    -> move-monitor left\n"
"bind Super+Shift+greater -> move-monitor right\n"
"bind Super+Escape  -> quit\n"
"\n"
"# Tap Super (quick press+release) toggles the app launcher.\n"
"bind tap Super -> toggle-launcher foot --app-id=kalin-launcher -e kalin-launch\n"
"# Hold Super for 1s (uninterrupted by another key) to show the window menu.\n"
"bind hold Super -> window-menu\n"
"\n"
"# Camera pan (arrows + vim keys)\n"
"bind Super+Shift+Left  -> viewport.pan -50 0\n"
"bind Super+Shift+Right -> viewport.pan 50 0\n"
"bind Super+Shift+Up    -> viewport.pan 0 -50\n"
"bind Super+Shift+Down  -> viewport.pan 0 50\n"
"bind Super+Shift+h -> viewport.pan -50 0\n"
"bind Super+Shift+l -> viewport.pan 50 0\n"
"bind Super+Shift+k -> viewport.pan 0 -50\n"
"bind Super+Shift+j -> viewport.pan 0 50\n"
"# Camera pan on Super + touchpad scroll (works on semi-MT pads, where\n"
"# libinput never offers 3-finger swipe/pinch gestures).\n"
"bind Super+ScrollUp    -> viewport.pan 0 -50\n"
"bind Super+ScrollDown  -> viewport.pan 0 50\n"
"bind Super+ScrollLeft  -> viewport.pan -50 0\n"
"bind Super+ScrollRight -> viewport.pan 50 0\n"
"bind Super+Ctrl+equal -> viewport.zoom 1.1\n"
"bind Super+Ctrl+minus -> viewport.zoom 0.9\n"
"bind Super+0         -> viewport.fit\n"
"bind Super+BackSpace -> viewport.reset\n"
"bind Super+z         -> viewport.follow\n"
"bind Super+Shift+z   -> viewport.follow-new\n"
"\n"
"# Directional focus\n"
"bind Super+Left  -> focus left\n"
"bind Super+Right -> focus right\n"
"bind Super+Up    -> focus up\n"
"bind Super+Down  -> focus down\n"
"\n"
"# Rail (layout Phase 2): 1D swap with the left/right rail neighbour on the\n"
"# freed Super+Ctrl+Arrow chord (Up/Down left unbound — the rail is 1D), and a\n"
"# discrete focus+frame the prev/next rail window on Super+Ctrl+h/l (the snap\n"
"# companion to the loose Super+scroll / Super+Shift+arrow camera pan).\n"
"bind Super+Ctrl+Left  -> rail-swap left\n"
"bind Super+Ctrl+Right -> rail-swap right\n"
"bind Super+Ctrl+h -> rail-focus left\n"
"bind Super+Ctrl+l -> rail-focus right\n"
"\n"
"# Attached overlay (layout Phase 5): arm the focused window as an overlay\n"
"# child, then click the window to pin it to as its host (it then follows the\n"
"# host on every move). On the freed Super+L; the direct `overlay-pin` IPC\n"
"# command is the scriptable path.\n"
"bind Super+l -> overlay-pin\n"
"\n"
"# Resize focused window: Super+=/- width, Super+Shift+=/- (plus/underscore) height\n"
"bind Super+equal  -> resize width 40\n"
"bind Super+minus  -> resize width -40\n"
"bind Super+Shift+plus       -> resize height 40\n"
"bind Super+Shift+underscore -> resize height -40\n"
"bind Super+bracketleft  -> resize width -40\n"
"bind Super+bracketright -> resize width 40\n"
"bind Super+Shift+braceleft  -> resize height -40\n"
"bind Super+Shift+braceright -> resize height 40\n"
"\n"
"# Pointer (drag to move/resize the focused window)\n"
"bind Super+BTN_LEFT       -> pointer-move\n"
"bind Super+BTN_RIGHT      -> pointer-resize\n"
"bind Super+Ctrl+BTN_RIGHT -> pointer-resize\n"
"# Grab the empty canvas background to drag-pan the camera (the world point\n"
"# under the cursor at grab-start stays under the cursor as you drag).\n"
"bind Super+Ctrl+BTN_LEFT  -> viewport.pan-grab\n"
"\n"
"# VT switching / emergency quit (do not remove)\n"
"bind Ctrl+Alt+Terminate_Server -> quit\n"
"bind Ctrl+Alt+XF86Switch_VT_1  -> chvt 1\n"
"bind Ctrl+Alt+XF86Switch_VT_2  -> chvt 2\n"
"bind Ctrl+Alt+XF86Switch_VT_3  -> chvt 3\n"
"bind Ctrl+Alt+XF86Switch_VT_4  -> chvt 4\n"
"bind Ctrl+Alt+XF86Switch_VT_5  -> chvt 5\n"
"bind Ctrl+Alt+XF86Switch_VT_6  -> chvt 6\n"
"bind Ctrl+Alt+XF86Switch_VT_7  -> chvt 7\n"
"bind Ctrl+Alt+XF86Switch_VT_8  -> chvt 8\n"
"bind Ctrl+Alt+XF86Switch_VT_9  -> chvt 9\n"
"bind Ctrl+Alt+XF86Switch_VT_10 -> chvt 10\n"
"bind Ctrl+Alt+XF86Switch_VT_11 -> chvt 11\n"
"bind Ctrl+Alt+XF86Switch_VT_12 -> chvt 12\n"
"\n"
"# Named-mode transitions (\"mode <name> { ... }\" blocks) aren't used by any\n"
"# default bind yet — the engine supports them (see bind_parser.c), nothing\n"
"# ships a mode block to enter one. Explicit so the coverage check doesn't\n"
"# read as an oversight.\n"
"unbind mode\n";

#endif /* KALIN_DEFAULT_BINDS_H */
